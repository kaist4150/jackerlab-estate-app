...
// 국토교통부 토지 실거래 API
const SALE_API_URL = 'https://apis.data.go.kr/1613000/RTMSDataSvcLandTrade/getRTMSDataSvcLandTrade';
...
function parseSaleItems(xmlItems: string[], district: string) {
  const items = xmlItems.map((itemXml, index) => {
    ...
    return {
      ...
      landUse: getXmlValue(itemXml, '용도지역', 'landUseCd'),
      landArea: parseFloat(getXmlValue(itemXml, '거래면적', 'dealArea')) || 0,
