...
import { Warehouse, Search, Calendar, MapPin, Loader2, AlertCircle } from 'lucide-react';
import { SEOUL_DISTRICTS } from '@/lib/constants';
import { formatPrice } from '@/lib/format';

interface FactoryTradeItem {
  id: string;
  name: string;
  district: string;
  dong: string;
...
export default function FactoryTradePage() {
  const [data, setData] = useState<FactoryTradeItem[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/trade/factory?${params}`);
  ...
  useEffect(() => {
    fetchData();
  }, [selectedDistrict, selectedYear, selectedMonth]);
  ...
  const filteredData = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      else if (sortBy === 'size') comparison = a.buildingArea - b.buildingArea;
    ...
    <div className="space-y-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">공장/창고 실거래가</h1>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr className="text-left text-sm text-gray-600">
                  <th className="px-4 py-3 font-semibold">건물용도</th>
                  ...
                  <th className="px-4 py-3 font-semibold cursor-pointer hover:text-emerald-600" onClick={() => handleSort('size')}>
                    건물면적 {sortBy === 'size' && (sortOrder === 'desc' ? '↓' : '↑')}
                  </th>
                  <th className="px-4 py-3 font-semibold">대지면적</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{item.buildingArea.toFixed(2)}㎡</td>
