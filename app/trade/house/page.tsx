...
export default function HouseTradePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/trade/house?${params}`);
  ...
  const stats = useMemo(() => {
    ...
    return { avg, max, min, count: filteredData.length };
  }, [filteredData]);
    ...
    <div className="space-y-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">연립다세대 실거래가</h1>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr className="text-left text-sm text-gray-600">
                  ...
                  <th
                    className="px-4 py-3 font-semibold cursor-pointer hover:text-emerald-600 text-right"
