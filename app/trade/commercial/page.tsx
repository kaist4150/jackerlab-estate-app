...
import { Store, Search, Calendar, MapPin, Loader2, AlertCircle } from 'lucide-react';
import { SEOUL_DISTRICTS } from '@/lib/constants';
import { formatPrice } from '@/lib/format';

interface CommercialTradeItem {
  id: string;
  name: string;
  district: string;
  dong: string;
...
export default function CommercialTradePage() {
  const [data, setData] = useState<CommercialTradeItem[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/trade/commercial?${params}`);
    ...
    <div className="space-y-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">상업/업무용 실거래가</h1>
        </div>
