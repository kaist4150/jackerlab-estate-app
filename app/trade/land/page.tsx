...
import { Mountain, Search, Calendar, MapPin, Loader2, AlertCircle } from 'lucide-react';
import { SEOUL_DISTRICTS } from '@/lib/constants';
import { formatPrice } from '@/lib/format';

interface LandTradeItem {
  id: string;
  district: string;
  dong: string;
  jibun: string;
...
export default function LandTradePage() {
  const [data, setData] = useState<LandTradeItem[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/trade/land?${params}`);
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      result = result.filter(item =>
        ...
        item.landUse.includes(searchTerm)
      );
    }
    ...
    <div className="space-y-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">토지 실거래가</h1>
        </div>
              ...
              <thead className="bg-gray-50">
                <tr className="text-left text-sm text-gray-600">
                  ...
                  <th className="px-4 py-3 font-semibold cursor-pointer hover:text-emerald-600" onClick={() => handleSort('size')}>
                    거래면적 {sortBy === 'size' && (sortOrder === 'desc' ? '↓' : '↑')}
                  </th>
                ...
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-900">{item.landUse || '-'}</div>
                    </td>
                    <td className="px-4 py-3">
